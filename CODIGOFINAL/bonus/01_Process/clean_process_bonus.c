...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   clean_process_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: thevaris <thevaris@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/20 16:32:38 by thevaris          #+#    #+#             */
/*   Updated: 2026/02/20 17:40:32 by thevaris         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3D_bonus.h"

static void	free_textures_map(t_mlx *mlx)
{
	free(mlx->west_texture.path);
	free(mlx->east_texture.path);
	free(mlx->south_texture.path);
	free(mlx->north_texture.path);
	if (mlx->door_tex.mlx_img == NULL)
		free(mlx->door_tex.path);
	if (mlx->map != NULL)
	{
...
void	ft_cleanup_and_exit(t_mlx *mlx)
{
	if (mlx->mlx_connect != NULL)
	{
		...
		if (mlx->door_tex.mlx_img)
			mlx_destroy_image(mlx->mlx_connect, mlx->door_tex.mlx_img);
		if (mlx->mlx_win != NULL)
